...
struct PlayerSeat: TableSeat {
    ...
    @MainActor static let seatPoses: [TableVisualState.Pose2D] = [
        .init(position: .init(x: 0, y: Double(GameMetrics.tableEdge)), rotation: .degrees(0)),
        .init(position: .init(x: -Double(GameMetrics.tableEdge), y: 0), rotation: .degrees(-90)),
        .init(position: .init(x: Double(GameMetrics.tableEdge), y: 0), rotation: .degrees(90))
    ]
    
    init(id: TableSeatIdentifier, pose: TableVisualState.Pose2D) {
        self.id = id
        let spatialSeatPose: TableVisualState.Pose2D = .init(position: .init(vector: pose.position.vector),
...
struct CardStockGroup: Equipment {
    ...
    init(id: ID) {
        ...
        initialState = .init(seatControl: .restricted([]),
                             pose: .init(position: .init(x: 0, y: -0.4), rotation: .degrees(90)),
...
struct CardGroup: EntityEquipment {
    ...
    init(id: ID, seat: PlayerSeat, root: Entity) {
        ...
        let handToSeat: TableVisualState.Pose2D = .init(position: .init(x: 0, y: -0.32), rotation: .init())
        let seatToTable = seat.initialState.pose
        let handToTable = handToSeat * seatToTable
        initialState = .init(seatControl: .restricted([]), pose: handToTable, entity: self.entity)
    ...
    func layoutChildren(for snapshot: TableSnapshot, visualState: TableVisualState) -> any EquipmentLayout {
        ...
        let leftPose: TableVisualState.Pose2D = .init(position: .init(x: -0.075, y: 0), rotation: .zero)
        let rightPose: TableVisualState.Pose2D = .init(position: .init(x: 0.075, y: 0), rotation: .zero)
...
struct ConveyorTile: Equipment, Sendable {
    ...
    @MainActor static let tiles: [(TableVisualState.Point2D, ConveyorTile.Category)] = [
        ...
        (.init(x: 0.04, y: 0.04), .grey),
        (.init(x: 0.2, y: 0), .grey),
        (.init(x: 0.35, y: 0), .green),
        (.init(x: 0.5, y: 0), .grey),
        (.init(x: 0.65, y: 0), .grey),
        (.init(x: 0.81, y: 0), .red),

        // Right column, top to bottom.
        (.init(x: 0.96, y: 0.04), .green),
        (.init(x: 1, y: 0.2), .grey),
        (.init(x: 1, y: 0.35), .green),
        (.init(x: 1, y: 0.5), .grey),
        (.init(x: 1, y: 0.65), .grey),
        (.init(x: 1, y: 0.81), .red),

        // Bottom row, right to left.
        (.init(x: 0.96, y: 0.96), .grey),
        (.init(x: 0.81, y: 1), .green),
        (.init(x: 0.65, y: 1), .grey),
        (.init(x: 0.5, y: 1), .red),
        (.init(x: 0.35, y: 1), .grey),
        (.init(x: 0.2, y: 1), .green),

        // Left column, bottom to top.
        (.init(x: 0.04, y: 0.96), .red),
        (.init(x: 0, y: 0.81), .grey),
        (.init(x: 0, y: 0.65), .green),
        (.init(x: 0, y: 0.5), .grey),
        (.init(x: 0, y: 0.35), .grey),
        (.init(x: 0, y: 0.2), .red)
    ]
...
struct PlayerPawn: EntityEquipment {
    ...
    init(id: ID, seat: PlayerSeat, cuteBotColor: CuteBotColor, entity: Entity) {
        ...
        self.entity = entity
        let pawnToSeat: TableVisualState.Pose2D = .init(position: .init(x: -0.2, y: -0.3), rotation: .init())
        let seatToTable = seat.initialState.pose
        let pawnToTable = pawnToSeat * seatToTable
        initialState = .init(pose: pawnToTable, entity: self.entity)
    }
}
...
struct Die: EntityEquipment {
    ...
    init(id: ID) {
        ...
        let pose: TableVisualState.Pose2D = .init(position: .init(x: -0.3, y: -0.5), rotation: .zero)
        initialState = .init(value: 1, seatControl: .any, pose: pose, entity: self.entity)
    }
